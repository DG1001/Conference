...
import { getEntities } from './timeslot.reducer';
    ...
    <div>
      ...
      <div className="table-responsive">
        {timeslotList && timeslotList.length > 0 ? (
            ...
            <tbody>
              {timeslotList.map((timeslot, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{timeslot.start ? <TextFormat type="date" value={timeslot.start} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{timeslot.end ? <TextFormat type="date" value={timeslot.end} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/timeslot/${timeslot.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/timeslot/${timeslot.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Keine Timeslots gefunden</div>
        )}
      </div>
      {totalItems ? (
        <div className={timeslotList && timeslotList.length > 0 ? '' : 'd-none'}>
